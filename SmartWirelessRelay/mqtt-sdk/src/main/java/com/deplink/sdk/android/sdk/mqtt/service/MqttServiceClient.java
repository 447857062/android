 ...
 */
    ...
    // fields for the connect definition
...
//
//                if (myDir == null) {
//                    resultBundle.putString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE, "No external storage available");
//                    resultBundle.putInt(MqttServiceConstants.CALLBACK_ERROR_NUMBER, -1);
...
//            }
            /**
             *在这里如果 myClient 不为空的时候说明当前已经存在了连接对象
             * 此时有两种情况（1.此时正在连接服务器 2.没有连接服务器->在没有连接服务器时有3中可能性：①没有网络已经被迫中断，②有了一个新的连接此连接被迫下线，
             * ③用户操作断开连接 ）
             *
             * 如果myClient为空的时候说明是初次连接，需要创建myClient对象
             *
             * 如果对象不为空，存在连接就是myClient.isConnected() 返回true的时候直接返回UI层连接成功
             *
             * 如果对象不为空，不存在连接就是myClient.isConnected() 返回fals的时候就需要进行服务器连接
             */
...
//
    ...
    private void attemptconnect(final Bundle resultBundle, String invocationContext) throws MqttException {
        ...
        IMqttActionListener listener = new MqttServiceClientListener(resultBundle) {
            ...
            public void onSuccess(IMqttToken asyncActionToken) {
                ...
                service.registerReceiver(pingSender, new IntentFilter(alarmAction));
                scheduleNextPing();
                service.callbackToActivity(clientHandle, Status.OK, resultBundle);
                deliverBacklog();
            }
    ...
    private void handleException(final Bundle resultBundle, Exception e) {
        ...
        resultBundle.putInt(MqttServiceConstants.CALLBACK_ERROR_NUMBER, (e instanceof MqttException) ? ((MqttException) e).getReasonCode() : -1);
        service.callbackToActivity(clientHandle, Status.ERROR, resultBundle);
    }
    ...
    public boolean isConnected() {
        return null != myClient && myClient.isConnected();
    }
    ...
    public IMqttDeliveryToken publish(String topic, byte[] payload, int qos,
        ...
        Log.i("debug","MqttServiceClient publish");
        final Bundle resultBundle = new Bundle();
        resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION,
    ...
    public void deliveryComplete(IMqttDeliveryToken messageToken) {
        ...
        // this notification will have kept the connect alive
    ...
    public void messageArrived(String topic, MqttMessage message)
        ...
        // receiving this message will have kept the connect alive for
    ...
    private void scheduleNextPing() {
        ...
        // the server, even if just with a ping, the connect is considered
        // broken.
        ...
        // thereby keeping the long-running connect open
    ...
    public class PingSender extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //billy: mqtt client will handle the keep alive processing according to mqtt protocol
            //the pingTopic is not allowed by the acl, sending to this topic will not get a complete
            //response so that not removed from the flying message stack, get to overflow and no more
